...
/**
 * @author blue
 * @description 亚马逊广告 For Sponsored Products（recordType=campaigns;segment=placement[location]） 报表 DAO
 * @email blue.liu@waymap.cn
 * @date 2019/10/31
 */
public interface SpCampaignsPlacementReportMapper extends BaseMapper<SpCampaignsPlacementReport> {
    /**
     * @MethodName: insertOrUpdateBatch
     * @Description: 批量插入或更新
     * 如果插入的记录导致一个UNIQUE索引或主键出现重复，那么就会认为该条记录存在，则执行更新语句，反之执行insert语句
     * @Param: [spCampaignsPlacementReportList]
     * @Return: void
     * @Author: blue
     * @Date: 2019/10/31
     **/
    void insertOrUpdateBatch(@Param("list") List<SpCampaignsPlacementReport> spCampaignsPlacementReportList);
    /**
    ...
    **/
     ...
     * @Date: 2019/11/1
    **/
    void updatePushResult(@Param("list") List<SpCampaignsPlacementReport> spCampaignsPlacementReportList);
}
